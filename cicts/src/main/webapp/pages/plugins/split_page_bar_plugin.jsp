...
2、传递的属性内容：
	...
	request.setAttribute("label","did");
...
<div id="pagebarDiv" style="float:right">
	<ul class="pagination">
		...
		%>
		<li><a href="<%=url%>?cp=<%=pageSize%>&ls=<%=lineSize%>&col=<%=column%>&kw=<%=keyword%>"><%=pageSize%></a></li>
		<%
				}
			}
		%>
		...
		%>
		<li><a href="<%=url%>?cp=<%=currentPage + 1%>&ls=<%=lineSize%>&col=<%=column%>&kw=<%=keyword%>">&gt;&gt;</a></li>
		<%
			}
		%>
	</ul>
